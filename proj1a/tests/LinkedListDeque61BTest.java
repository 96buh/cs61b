import org.junit.jupiter.api.Test;

import static com.google.common.truth.Truth.assertThat;
import static com.google.common.truth.Truth.assertWithMessage;

/** Performs some basic linked list tests. */
public class LinkedListDeque61BTest {

     @Test
     /** In this test, we have three different assert statements that verify that addFirst works correctly. */
     public void addFirstTestBasic() {
         Deque61B<String> lld1 = new LinkedListDeque61B<>();

         lld1.addFirst("back"); // after this call we expect: ["back"]
         assertThat(lld1.toList()).containsExactly("back").inOrder();

         lld1.addFirst("middle"); // after this call we expect: ["middle", "back"]
         assertThat(lld1.toList()).containsExactly("middle", "back").inOrder();

         lld1.addFirst("front"); // after this call we expect: ["front", "middle", "back"]
         assertThat(lld1.toList()).containsExactly("front", "middle", "back").inOrder();

         /* Note: The first two assertThat statements aren't really necessary. For example, it's hard
            to imagine a bug in your code that would lead to ["front"] and ["front", "middle"] failing,
            but not ["front", "middle", "back"].
          */
     }

     @Test
     /** In this test, we use only one assertThat statement. IMO this test is just as good as addFirstTestBasic.
      *  In other words, the tedious work of adding the extra assertThat statements isn't worth it. */
     public void addLastTestBasic() {
         Deque61B<String> lld1 = new LinkedListDeque61B<>();

         lld1.addLast("front"); // after this call we expect: ["front"]
         lld1.addLast("middle"); // after this call we expect: ["front", "middle"]
         lld1.addLast("back"); // after this call we expect: ["front", "middle", "back"]
         assertThat(lld1.toList()).containsExactly("front", "middle", "back").inOrder();
     }

     @Test
     /** This test performs interspersed addFirst and addLast calls. */
     public void addFirstAndAddLastTest() {
         Deque61B<Integer> lld1 = new LinkedListDeque61B<>();

         /* I've decided to add in comments the state after each call for the convenience of the
            person reading this test. Some programmers might consider this excessively verbose. */
         lld1.addLast(0);   // [0]
         lld1.addLast(1);   // [0, 1]
         lld1.addFirst(-1); // [-1, 0, 1]
         lld1.addLast(2);   // [-1, 0, 1, 2]
         lld1.addFirst(-2); // [-2, -1, 0, 1, 2]

         assertThat(lld1.toList()).containsExactly(-2, -1, 0, 1, 2).inOrder();
     }

     // Below, you'll write your own tests for LinkedListDeque61B.
     @Test
     public void myAddFirstTest() {
         Deque61B<String> lld1 = new LinkedListDeque61B<>();

         lld1.addFirst("back"); // after this call we expect: ["front"]
         lld1.addFirst("middle"); // after this call we expect: ["middle", "back]
         lld1.addFirst("front"); // after this call we expect: ["front", "middle", "back"]
         assertThat(lld1.toList()).containsExactly("front", "middle", "back").inOrder();
     }

     @Test
    public void sizeTest() {
         Deque61B<String> lld1 = new LinkedListDeque61B<>();

         assertThat(lld1.isEmpty()).isTrue();
         assertThat(lld1.size()).isEqualTo(0);

         lld1.addFirst("hello");
         assertThat(lld1.isEmpty()).isFalse();
         assertThat(lld1.size()).isEqualTo(1);

         lld1.addFirst("33");
         lld1.addLast("bart");
         lld1.addFirst("marge");
         assertThat(lld1.size()).isEqualTo(4);
     }

     @Test
    public void getTest() {
         Deque61B<Integer> lld1 = new LinkedListDeque61B<>();

         assertThat(lld1.get(0)).isNull();
         assertThat(lld1.get(1)).isNull();

         lld1.addFirst(1);
         lld1.addLast(2);
         lld1.addFirst(3); // [3, 1, 2]
         assertThat(lld1.get(2)).isEqualTo(2);

         lld1.addFirst(5); // [5, 3, 1, 2]
         assertThat(lld1.get(0)).isEqualTo(5);
         assertThat(lld1.get(3)).isEqualTo(2);
         assertThat(lld1.get(1)).isEqualTo(3);
     }

     @Test void getOOBTest() {
         Deque61B<Integer> lld1 = new LinkedListDeque61B<>();

         assertThat(lld1.get(0)).isNull();
         assertThat(lld1.get(1)).isNull();
         assertThat(lld1.get(-5)).isNull();

         lld1.addFirst(1);
         lld1.addLast(2);
         lld1.addFirst(3); // [3, 1, 2]

         assertThat(lld1.get(10)).isNull();
         assertThat(lld1.get(-1)).isNull();
     }

    @Test
    public void getRecursiveTest() {
         Deque61B<Integer> lld1 = new LinkedListDeque61B<>();

         assertThat(lld1.getRecursive(0)).isNull();
         assertThat(lld1.getRecursive(1)).isNull();

         lld1.addFirst(1);
         lld1.addLast(2);
         lld1.addFirst(3); // [3, 1, 2]
         assertThat(lld1.getRecursive(2)).isEqualTo(2);

         lld1.addFirst(5); // [5, 3, 1, 2]
         assertThat(lld1.getRecursive(0)).isEqualTo(5);
         assertThat(lld1.getRecursive(3)).isEqualTo(2);
         assertThat(lld1.getRecursive(1)).isEqualTo(3);
     }

     @Test
    public void removeFirstTest() {
         Deque61B<Integer> lld1 = new LinkedListDeque61B<>();
         lld1.addFirst(1);
         lld1.addLast(2);
         lld1.addFirst(3); // [3, 1, 2]
         assertThat(lld1.toList()).containsExactly(3, 1, 2).inOrder();


         assertThat(lld1.size()).isEqualTo(3);

         lld1.removeFirst(); // [1, 2]
         assertThat(lld1.toList()).containsExactly(1, 2).inOrder();
         assertThat(lld1.size()).isEqualTo(2);
         lld1.removeFirst(); // [2]
         assertThat(lld1.size()).isEqualTo(1);
         assertThat(lld1.isEmpty()).isFalse();

         lld1.addFirst(5);
         lld1.addFirst(8);
         lld1.addFirst(7); // [7, 8, 5, 2]
         assertThat(lld1.toList()).containsExactly(7, 8, 5, 2).inOrder();

         lld1.removeFirst();
         lld1.removeFirst();
         lld1.removeFirst();
         lld1.removeFirst();
         assertThat(lld1.size()).isEqualTo(0);
         assertThat(lld1.isEmpty()).isTrue();
     }

    @Test
    public void removeLastTest() {
        Deque61B<Integer> lld1 = new LinkedListDeque61B<>();
        lld1.addFirst(1);
        lld1.addLast(2);
        lld1.addFirst(3); // [3, 1, 2]
        assertThat(lld1.toList()).containsExactly(3, 1, 2).inOrder();


        assertThat(lld1.size()).isEqualTo(3);

        lld1.removeLast(); // [3, 1]
        assertThat(lld1.toList()).containsExactly(3, 1).inOrder();
        assertThat(lld1.size()).isEqualTo(2);
        lld1.removeLast(); // [3]
        assertThat(lld1.size()).isEqualTo(1);
        assertThat(lld1.isEmpty()).isFalse();

        lld1.addFirst(5);
        lld1.addFirst(8);
        lld1.addFirst(7); // [7, 8, 5, 3]
        assertThat(lld1.toList()).containsExactly(7, 8, 5, 3).inOrder();

        lld1.removeLast();
        lld1.removeLast();
        lld1.removeLast();
        lld1.removeLast();
        assertThat(lld1.size()).isEqualTo(0);
        assertThat(lld1.isEmpty()).isTrue();
    }

    @Test
    public void removeFirstAndLastTest() {
        Deque61B<Integer> lld1 = new LinkedListDeque61B<>();
        lld1.addFirst(1);
        lld1.addLast(2);
        lld1.addFirst(3); // [3, 1, 2]
        assertThat(lld1.toList()).containsExactly(3, 1, 2).inOrder();


        assertThat(lld1.size()).isEqualTo(3);

        lld1.removeLast(); // [3, 1]
        assertThat(lld1.toList()).containsExactly(3, 1).inOrder();
        assertThat(lld1.size()).isEqualTo(2);
        lld1.removeFirst(); // [1]
        assertThat(lld1.size()).isEqualTo(1);
        assertThat(lld1.isEmpty()).isFalse();

        lld1.addFirst(5);
        lld1.addFirst(8);
        lld1.addFirst(7); // [7, 8, 5, 1]
        assertThat(lld1.toList()).containsExactly(7, 8, 5, 1).inOrder();
        lld1.addLast(10); // [7, 8, 5, 1, 10]

        lld1.removeLast(); // [7, 8, 5, 1]
        lld1.removeFirst(); // [8, 5, 1]
        assertThat(lld1.toList()).containsExactly(8, 5, 1).inOrder();
        lld1.removeFirst();
        lld1.removeLast();
        assertThat(lld1.toList()).containsExactly(5).inOrder();
        lld1.removeLast();
        assertThat(lld1.size()).isEqualTo(0);
        assertThat(lld1.isEmpty()).isTrue();
    }

    @Test
    public void removeFirstLastEmptyTest() {
         Deque61B<Integer> lld1 = new LinkedListDeque61B<>();

         assertThat(lld1.removeFirst()).isNull();
         assertThat(lld1.removeLast()).isNull();
    }

    @Test
    public void sizeAfterRemoveFromEmptyTest() {
        Deque61B<Integer> lld1 = new LinkedListDeque61B<>();
        assertThat(lld1.size()).isEqualTo(0);
        lld1.removeFirst();
        assertThat(lld1.size()).isEqualTo(0);
        lld1.addFirst(1);
        lld1.addFirst(2);
        assertThat(lld1.size()).isEqualTo(2);
        lld1.removeFirst();
        lld1.removeFirst();
        lld1.removeFirst();
        assertThat(lld1.size()).isEqualTo(0);
    }

    @Test
    public void toListEmptyTest() {
        Deque61B<Integer> lld1 = new LinkedListDeque61B<>();
        assertThat(lld1.toList()).containsExactly().inOrder();
    }

    @Test
    public void toListNonEmptyTest() {
        Deque61B<Integer> lld1 = new LinkedListDeque61B<>();
        assertThat(lld1.toList()).containsExactly().inOrder();

        lld1.addFirst(5);
        lld1.addFirst(1);
        lld1.addLast(7);
        lld1.addFirst(26); // [26, 1, 5, 7]
        assertThat(lld1.toList()).containsExactly(26, 1, 5, 7).inOrder();
    }

    @Test
    public void isEmptyTest() {
        Deque61B<Integer> lld1 = new LinkedListDeque61B<>();
        assertThat(lld1.isEmpty()).isTrue();
        lld1.addLast(1);
        assertThat(lld1.isEmpty()).isFalse();
    }

    @Test
    public void integrationTest() {
        Deque61B<Integer> lld1 = new LinkedListDeque61B<>();
        assertThat(lld1.isEmpty()).isTrue();
        assertThat(lld1.size()).isEqualTo(0);
        assertThat(lld1.toList()).containsExactly().inOrder();
        assertThat(lld1.get(0)).isNull();

        lld1.addFirst(5);
        assertThat(lld1.isEmpty()).isFalse();
        assertThat(lld1.size()).isEqualTo(1);
        assertThat(lld1.toList()).containsExactly(5).inOrder();

        lld1.addFirst(5); // [5, 5]
        lld1.addFirst(8); // [8, 5, 5]
        lld1.addLast(27); // [8, 5, 5, 27]
        lld1.addLast(3); // [8, 5, 5, 27, 3]
        lld1.addFirst(29); // [29, 8, 5, 5, 27, 3]
        assertThat(lld1.size()).isEqualTo(6);
        assertThat(lld1.toList()).containsExactly(29, 8, 5, 5, 27, 3).inOrder();

        assertThat(lld1.get(0)).isEqualTo(29);
        assertThat(lld1.get(3)).isEqualTo(5);
        assertThat(lld1.get(5)).isEqualTo(3);
        assertThat(lld1.get(6)).isNull();
        assertThat(lld1.get(100)).isNull();
        assertThat(lld1.get(-1)).isNull();

        lld1.removeFirst(); // [8, 5, 5, 27, 3]
        lld1.removeFirst(); // [5, 5, 27, 3]
        lld1.removeLast(); // [5, 5, 27]
        assertThat(lld1.size()).isEqualTo(3);
        assertThat(lld1.toList()).containsExactly(5, 5, 27).inOrder();
        assertThat(lld1.isEmpty()).isFalse();

        lld1.removeLast(); // [5, 5]
        lld1.removeLast(); // [5]
        assertThat(lld1.get(0)).isEqualTo(5);
        assertThat(lld1.get(1)).isNull();
        assertThat(lld1.size()).isEqualTo(1);
        assertThat(lld1.toList()).containsExactly(5).inOrder();
        assertThat(lld1.isEmpty()).isFalse();

        lld1.removeFirst();
        assertThat(lld1.size()).isEqualTo(0);
        assertThat(lld1.toList()).containsExactly().inOrder();
        assertThat(lld1.isEmpty()).isTrue();

        assertThat(lld1.removeFirst()).isNull();
        assertThat(lld1.removeLast()).isNull();
        assertThat(lld1.get(0)).isNull();
    }
}