...
public class TestOneWordKnot0Hyponyms {
    ...
    @Test
    public void testSingleWordMultiQueries() {
        NgordnetQueryHandler studentHandler = AutograderBuddy.getHyponymsHandler(
                FREQUENCY_EECS_FILE, TOTAL_COUNTS_FILE, SYNSETS_EECS_FILE, HYPONYMS_EECS_FILE);
        List<String> words = new ArrayList<>();
        words.add("EE105");
        ...
        NgordnetQuery nq = new NgordnetQuery(words, 2010, 2020, 5);
        String actual = studentHandler.handle(nq);
        String expected = "[EE105, EE113, EE130, EE140, EE142]";
        assertThat(actual).isEqualTo(expected);

        List<String> words2 = new ArrayList<>();
        words2.add("CS168");

        NgordnetQuery nq2 = new NgordnetQuery(words2, 2010, 2020, 4);
        String actual2 = studentHandler.handle(nq2);
        String expected2 = "[CS168]";
        assertThat(actual2).isEqualTo(expected2);
    }

    // 測試返回的words小於k
    @Test
    public void testFewerThanK() {
        NgordnetQueryHandler studentHandler = AutograderBuddy.getHyponymsHandler(
                FREQUENCY_EECS_FILE, TOTAL_COUNTS_FILE, SYNSETS_EECS_FILE, HYPONYMS_EECS_FILE);
        List<String> words = new ArrayList<>();
        words.add("EE117");
